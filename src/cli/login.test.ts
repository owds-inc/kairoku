...
function fakeApp(mode: Mode = "happy") {
  const calls: string[] = [];
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (req.method === "GET" && url.pathname === "/.well-known/oauth-protected-resource/api/mcp") {
        return Response.json({ resource: mode === "resourceMismatch" ? "https://wrong.example/api/mcp" : RESOURCE });
      }
      if (req.method === "POST" && url.pathname === "/api/mcp") {
        ...
        if (body.method === "tools/list") {
          return Response.json({ jsonrpc: "2.0", id: body.id, result: { tools: [{ name: "a" }, { name: "b" }, { name: "c" }] } });
        }
...
describe("kairoku login — verify before write", () => {
  ...
  test("metadata resource mismatch: nothing persisted, nonzero exit", async () => {
    const app = fakeApp("resourceMismatch");
    servers.push(app);
    const io = testIo();
  ...
  test("401 on the round trip: nothing persisted, nonzero exit", async () => {
    const app = fakeApp("unauthorized");
    servers.push(app);
    const io = testIo();
