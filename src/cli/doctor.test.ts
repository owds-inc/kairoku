...
describe("kairoku doctor", () => {
  ...
  test("§3B — kairoku login present + bridge initialize reachable is a PASS", async () => {
    const io = linuxDaemon();
    io.files["/home/tester/.kairoku/config.json"] = JSON.stringify({
      listen: { host: "10.0.0.5", port: 7801 },
      appUrl: "https://app.test",
    ...
    const list = await checks(io, { probe: () => true });
    expect(byName(list, "kairoku login (MCP access)")).toMatchObject({ status: "PASS" });
    expect(byName(list, "kairoku mcp-bridge reachable")).toMatchObject({ status: "PASS" });
  });

  test("§3B — a revoked MCP token FAILs the bridge check, naming the fix", async () => {
    const io = linuxDaemon();
    io.files["/home/tester/.kairoku/config.json"] = JSON.stringify({
      listen: { host: "10.0.0.5", port: 7801 },
      appUrl: "https://app.test",
  ...
  test("§3B — a stale codex `--url .../api/mcp` entry is a WARN naming `kairoku mcp setup`", async () => {
    const io = linuxDaemon();
    io.canned["codex mcp get kairoku --json"] = { stdout: JSON.stringify({ transport: { url: "https://kairoku.io/api/mcp" } }) };
    const list = await checks(io, { probe: () => true });
    expect(byName(list, "codex MCP entry is the stale URL form")).toMatchObject({
      status: "WARN",
      detail: expect.stringContaining("kairoku mcp setup --agent codex"),
    });
  });
